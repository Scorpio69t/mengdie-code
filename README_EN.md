...
The interactive entry accepts one task of at most 64 KiB per process. Reconstructable boundaries are committed to local SQLite before output, while streaming `message.delta` events remain transient. `session resume` creates a new Run in the same Session and restores complete user, assistant, read-only tool, and Todo boundaries; write, execute, and network results are replaced by recovery-safe summaries. A pending approval can resume only in an interactive terminal: MengDie Code re-prepares against the current project state, presents a fresh preview, and requires a new decision; the prior Capability is never reused. An in-flight read/state call also requires explicit confirmation before retry. Unknown write/execute/network state, multiple incomplete calls, missing context, and mismatched private/public facts fail closed. Patch Journal, TUI, and REPL are not implemented yet; redirected input/output must use `mengdie exec`.
