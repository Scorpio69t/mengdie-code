...
func TestOpenSQLiteAppliesSchemaAndConnectionSettings(t *testing.T) {
	...
	for _, table := range []string{"schema_migrations", "sessions", "runs", "events", "commands", "snapshots", "context_messages", "artifacts", "context_summaries"} {
		var count int
...
func TestArtifactMigrationUpgradesExistingContextLedger(t *testing.T) {
	...
	if migrationCount != 5 || artifactColumnCount != 1 {
