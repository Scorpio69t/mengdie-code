...
视觉只使用一枚“梦蝶玉”强调色，其余信息依靠字重、边框和间距建立层级，`NO_COLOR` 下仍完整可读。SVG 与透明 PNG 保留由两组代码尖括号和断开插入光标组成的单色“代码蝶”。第二次审核提供的 2048px Windows Terminal 实机截图证明，斜线字符画会受字体单元格比例影响而散架，因此终端改用跨字体稳定的 ASCII 标记 `<>|<>`，不再把矢量轮廓生硬像素化。
...
- `go fmt ./...`、`go vet ./...`、`go test ./...`：通过，463 项 / 18 个包；`go test -race ./...` 同样通过；
