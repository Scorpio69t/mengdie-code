...
- `internal/policy`：
  ...
  - `Authorizer` 防御性快照、工作目录真实路径归一化、30 秒默认 TTL、256 bit 加密随机 nonce；
...
- `go test ./...`：14 个包、248 项测试通过；
- `go test -race ./internal/policy ./internal/tools`：113 项定向测试通过；
- `go vet ./...`：通过；
- `gofmt -l .` 与 `git diff --check`：无输出。
- `mengdie-eval --manifest evals/coding/smoke.json --pretty`：5/5 baseline 通过。

定向回归覆盖规则默认矩阵与四层优先级、硬拒绝不可覆盖、无交互不调用 Broker、审批允许/拒绝/编辑、审批事件前后失败、事件敏感信息不泄漏、超长和无效输入、拒绝零副作用、真实 `read_file` 授权执行，以及 Capability 的全部安全绑定。RunID、工具、规范参数、digest、cwd、路径和敏感标记、effect、前置条件、过期时间与 nonce 均有篡改用例；32 路并发消费只有一次成功。另有路径别名回归，确保 macOS `/var` 与 `/private/var`、Windows 临时目录别名等场景在 Policy、PathGuard 与 Capability 中使用同一真实根目录。
