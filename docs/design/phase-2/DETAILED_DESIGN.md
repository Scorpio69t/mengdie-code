...
- 受控写前镜像的大小、SHA-256 和归属重新验证通过；
- 当前权限位严格等于 Journal 的写后权限位；
- 用户显式确认高风险删除或覆盖。
...
- P2-06B（已实现）：私有回滚材料、幂等 rewind Command、强制审批、权限位检查和用户后续编辑保护；
